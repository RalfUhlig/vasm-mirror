...
static int listbpl=8,listnoinc;
static int listformat,listtitlecnt;
...
static size_t get_symbols(symbol **symlist,uint32_t flags)
{
  ...
  for (cnt=0,sym=first_symbol; sym; sym=sym->next) {
    if ((sym->flags & VASMINTERN) || *sym->name==' ')
      continue;
    if (sym->type!=EXPRESSION || (sym->flags & flags)) {
      cnt++;
      if (symlist != NULL)
        *symlist++ = sym;
    }
...
static void write_listing_std(char *listname,section *first_section)
{
  FILE *f;
...
static void write_listing_wide(char *listname,section *first_section)
{
  ...
  if (!listnosyms) {
    size_t nsyms = get_symbols(NULL,USED|EXPORT|COMMON|WEAK|LOCAL|ABSLABEL);
    symbol *sym,**symlist;

    fprintf(f,"\n\nSymbols by name:\n");
    symlist = (symbol **)mymalloc(nsyms*sizeof(symbol *));
    get_symbols(symlist,USED|EXPORT|COMMON|WEAK|LOCAL|ABSLABEL);
